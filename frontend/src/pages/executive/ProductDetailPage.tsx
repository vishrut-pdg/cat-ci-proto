...
import type { CostDriversResponse, ProductDetailResponse, ProductTrendResponse } from "../../types/executive";
...
export default function ProductDetailPage() {
  ...
  const insight = useMemo(() => detail && topDriver ? `${topDriver.driver_name} explains ${percent(Number(topDriver.contribution_percent))} of the structured cost gap. ${detail.highest_cost_plant} is the highest-cost plant; start the drill-down with ${detail.components[0]?.component_name ?? "the leading component"}.` : "", [detail, topDriver]);
